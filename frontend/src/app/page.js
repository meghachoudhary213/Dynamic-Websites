...
import { 
  ...
  UserPlus, UserCheck, ShieldCheck, Mail, Phone, Clock, Laptop, Trash2,
  Award, ShoppingBag, Heart, Shield
} from 'lucide-react';
...
export default function Home() {
  ...
  const [activeTab, setActiveTab] = useState('website_manager'); // website_manager | theme_manager | content_manager | analytics | users | settings
  const [previewDevice, setPreviewDevice] = useState('desktop'); // desktop | tablet | mobile
  const [lightMode, setLightMode] = useState(false);
  const [soundEnabled, setSoundEnabled] = useState(true);

  // Selected website card state
  const [selectedWebsiteCard, setSelectedWebsiteCard] = useState('NextRank Institute');

  // Active customizer form clone
  const [formConfig, setFormConfig] = useState(null);

  // Deploy New Dynamic Website wizard state
  const [saasName, setSaasName] = useState('Nexus Academy');
  const [saasCategory, setSaasCategory] = useState('coaching');
  const [saasPrimary, setSaasPrimary] = useState('#6366f1');
  ...
  const [paymentUser, setPaymentUser] = useState('Ramesh Patel (Vijay Nagar)');
  const [paymentPlan, setPaymentPlan] = useState('Premium Enrollment Plan');
  ...
  useEffect(() => {
    ...
    const category = activeConfig.category || (activeConfig.businessType && activeConfig.businessType.includes('_') ? activeConfig.businessType.split('_')[0] : activeConfig.businessType) || 'coaching';
    
    const root = document.documentElement;
    if (category === 'coaching' || category === 'education') {
      // Dark blue glassmorphism theme for parent dashboard
      root.style.setProperty('--primary', '#3b82f6'); // Neon Blue
      root.style.setProperty('--secondary', '#6366f1'); // Indigo
      root.style.setProperty('--accent', '#00f5ff'); // Cyan
    } else if (category === 'ecommerce') {
      // Orange premium commerce theme for parent dashboard
      root.style.setProperty('--primary', '#f97316'); // Commerce Orange
      root.style.setProperty('--secondary', '#fbbf24'); // Gold
      root.style.setProperty('--accent', '#ef4444'); // Coral
    } else {
      // Standard configuration default styling
      root.style.setProperty('--primary', activeConfig.theme?.primary || '#6366f1');
      root.style.setProperty('--secondary', activeConfig.theme?.secondary || '#a855f7');
      root.style.setProperty('--accent', activeConfig.theme?.accent || '#f43f5e');
    }
  }, [activeConfig]);
  ...
  const handleCreateSaaSWebsite = async (e) => {
    ...
    try {
      const defaultSectionsMap = {
        coaching: ['courses', 'faculty', 'results'],
        ecommerce: ['products', 'offers', 'categories'],
        real_estate: ['properties', 'regions', 'agents'],
        hospital: ['services', 'doctors', 'appointments'],
        cafe: ['menu', 'gigs', 'gallery'],
        startup: ['programs', 'mentors', 'showcase'],
        gym: ['workouts', 'trainers', 'plans'],
        tourism: ['packages', 'stays', 'itineraries'],
        cybersecurity: ['quizzes', 'tips', 'threats'],
        career: ['internships', 'skills', 'success'],
        smartengine: ['analytics', 'workflows', 'recommendations']
      };

      const defaultModulesMap = {
        coaching: ['students', 'attendance', 'tests', 'notes', 'fees', 'analytics'],
        ecommerce: ['inventory', 'orders', 'sales_ledger', 'shipping'],
        real_estate: ['listings', 'bookings', 'leads', 'agents'],
        hospital: ['patients', 'schedules', 'billing', 'records'],
        cafe: ['menu_editor', 'orders', 'reservations', 'feedback'],
        startup: ['members', 'repo', 'builds', 'analytics'],
        gym: ['members', 'sessions', 'diets', 'payments'],
        tourism: ['bookings', 'leads', 'itinerary_builder', 'billing'],
        cybersecurity: ['threats', 'logs', 'firewall', 'pentest'],
        career: ['applicants', 'jobs', 'interviews', 'skills'],
        smartengine: ['orchestration', 'models', 'vectors', 'compute']
      };
      ...
      const res = await fetch(`${API_URL}/api/website/create`, {
        ...
        body: JSON.stringify({
          ...
          fonts: saasFont,
          sectionsSelection: defaultSectionsMap[saasCategory] || ['hero', 'features'],
          dashboardModulesSelection: defaultModulesMap[saasCategory] || ['analytics']
        })
      });
      const data = await res.json();
      if (data.success) {
        alert(`🎉 Dynamic website "${saasName}" successfully deployed! The engine has switched!`);
        setActiveConfig(data.config);
        setFormConfig(JSON.parse(JSON.stringify(data.config)));
        loadAllConfigurations();
        setTimeout(() => {
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      ...
      if (data.success) {
        setActiveConfig(data.config);
        alert('Configuration successfully written to MongoDB database!');
        setTimeout(() => {
          reloadPreviewIframe();
        }, 300);
      }
  ...
  const handleTriggerAISuggestTheme = async (styleMode) => {
    ...
    try {
      ...
      if (data.success && data.theme) {
        ...
        setFormConfig(updated);
        alert(`Suggested HSL color theme "${styleMode}" applied to visual layout! Click Save Styling to write.`);
      }
    } catch (err) {
  ...
  const handleManualPaymentSimulation = async (e) => {
    e.preventDefault();
    playClickSound();
    try {
      const cleanPrice = parseInt(paymentAmount.replace(/[^0-9]/g, '')) || 15000;
      ...
      const data = await res.json();
      if (data.success) {
        alert(`💳 Simulated Razorpay Webhook Broadcasted successfully!\nTransaction ID: ${data.transactionId}\nWebhook logged!`);
        loadNotifications();
      }
    } catch (err) {
  ...
  const handleProvisionUser = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        alert(`🎉 User "${newUserName}" provisioned successfully with role "${newUserRole}"!`);
        setNewUserName('');
        setNewUserEmail('');
        setNewUserPhone('');
        loadUsers();
      ...
      alert('Error provisioning user.');
    }
  };

  if (loading) {
    return (
      <div className="flex bg-slate-950 flex-col items-center justify-center text-white h-screen font-mono">
        <RefreshCw className="w-12 h-12 text-indigo-500 animate-spin mb-4" />
        <p className="text-sm font-medium tracking-wide">Validating Command session...</p>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 font-sans relative overflow-hidden">
        ...
        <div className="w-full max-w-md bg-slate-900/60 backdrop-blur-xl rounded-3xl border border-white/10 p-8 space-y-6 shadow-2xl relative overflow-hidden animate-in zoom-in-95 duration-500">
          ...
          <div className="text-center space-y-2 relative">
            <div className="w-14 h-14 rounded-2xl bg-indigo-600 flex items-center justify-center mx-auto mb-4 shadow-xl shadow-indigo-500/30">
              <Lock className="w-6 h-6 text-white" />
            </div>
            <h2 className="text-2xl font-black tracking-tight text-white uppercase font-mono">Nexus Command Center</h2>
            <p className="text-xs text-slate-400 font-mono">Centralized Website CMS Dashboard Console</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              <input 
                ...
                className="w-full bg-slate-950 border border-white/10 text-xs px-4 py-3.5 rounded-xl text-white focus:outline-none focus:border-indigo-500/50" 
              />
            </div>
            <div>
  ...
  return (
    <div className={lightMode ? 'min-h-screen flex flex-col bg-slate-50 text-slate-900 transition-all duration-300' : 'min-h-screen flex flex-col bg-slate-950 text-slate-100 transition-all duration-300'}>
      ...
      {/* Header bar */}
      <header className="h-16 border-b border-white/5 bg-slate-900/60 backdrop-blur-md px-6 flex items-center justify-between sticky top-0 z-30 font-sans">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2">
            <span className="w-2.5 h-2.5 bg-emerald-500 rounded-full animate-ping" />
            <h1 className="font-mono font-black text-sm tracking-widest uppercase text-indigo-400 flex items-center gap-2">
              <Bot className="w-5 h-5" /> Nexus Command Center
            </h1>
          </div>
      ...
      {/* Main command center workspace layout */}
      <div className="flex-1 grid grid-cols-1 xl:grid-cols-12 overflow-hidden h-[calc(100vh-64px)] font-sans">
        
        {/* Left Sidebar Pane (4 columns) */}
        <div className="xl:col-span-4 border-r border-white/5 flex flex-col bg-slate-900/10 overflow-y-auto">
          
          {/* Centralized Workspace menu selectors */}
          <div className="grid grid-cols-3 border-b border-white/5 bg-slate-900/40 p-2 gap-1 overflow-x-auto shrink-0 font-mono">
            {[
              { id: 'website_manager', label: 'Website Manager', icon: Grid },
              { id: 'theme_manager', label: 'Theme Manager', icon: Palette },
              { id: 'content_manager', label: 'Content Manager', icon: Layers },
              ...
              return (
                <button
                  ...
                  className={`flex flex-col items-center justify-center p-2 rounded-xl text-[9px] font-bold uppercase transition-all gap-1 border ${
                    activeTab === tab.id 
          ...
          <div className="p-6 flex-1 space-y-6 overflow-y-auto">
            ...
            {/* TAB 1: WEBSITE MANAGER */}
            {activeTab === 'website_manager' && (
              <div className="space-y-6 animate-in fade-in duration-300 text-left">
                <div className="space-y-1">
                  <h3 className="font-bold text-sm text-indigo-400 font-mono uppercase tracking-wider">Website Manager</h3>
                  <p className="text-[10px] opacity-80 text-slate-400">Toggle active dynamic templates or provision a fresh engine profile in MongoDB.</p>
                </div>

                {/* Website Selector System Cards */}
                <div className="space-y-3 p-4 bg-indigo-950/20 border border-indigo-500/10 rounded-2xl">
                  <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-wider block font-mono">Website Selector System</span>
                  <p className="text-[9px] text-slate-400">Click a card to select the website, store it in state, and dynamically swap the preview configuration.</p>
                  
                  <div className="space-y-2">
                    {[
                      { name: 'NextRank Institute', desc: 'Educational & parent notification network.', category: 'coaching', icon: Award },
                      { name: 'ShopVerse', desc: 'Sleek luxury e-commerce boutique system.', category: 'ecommerce', icon: ShoppingBag },
                      { name: 'MediCare', desc: 'High-fidelity healthcare clinical portal.', category: 'hospital', icon: Heart },
                      { name: 'CyberShield', desc: 'Intrusion analysis matrix & threat network.', category: 'cybersecurity', icon: Shield }
                    ].map((site) => {
                      const SiteIcon = site.icon;
                      const isSelected = selectedWebsiteCard === site.name;
                      return (
                        <button
                          key={site.name}
                          onClick={() => {
                            playClickSound();
                            setSelectedWebsiteCard(site.name);
                            handleSwitchTemplate(site.category);
                          }}
                        ...
                        >
                          <div className={`p-2 rounded-lg shrink-0 mt-0.5 ${
                            isSelected ? 'bg-indigo-500 text-white' : 'bg-slate-950 text-slate-400'
                          }`}>
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center justify-between">
                              ...
                              {isSelected && (
                                <span className="text-[8px] bg-indigo-500/20 text-indigo-400 border border-indigo-500/30 px-1.5 py-0.5 rounded-md font-mono uppercase font-bold">
                                  Selected
                                </span>
                              )}
                            </div>
                            <p className="text-[9px] text-slate-400 mt-0.5 leading-tight">{site.desc}</p>
                          </div>
                        </button>
                      );
                    })}
                ...
                <div className="space-y-2.5">
                  <span className="text-[9px] font-bold text-slate-500 uppercase tracking-wider block font-mono">Select Active Configured Profile</span>
                  <div className="grid grid-cols-2 gap-2">
                    {allConfigs.map((cfg) => (
                      ...
                      >
                        <span className="text-[8px] font-mono uppercase text-indigo-400 block truncate">
                          {cfg.businessType.replace('_', ' ')}
                        </span>
                        <span className="text-[11px] font-bold block mt-1 leading-tight text-white truncate">
                          {cfg.theme.name}
                        </span>
                        <span className="text-[9px] opacity-60 font-mono text-slate-400 block mt-0.5 capitalize">
                          {cfg.category || cfg.businessType.split('_')[0]}
                        </span>
                        {cfg.isActive && (
                          <span className="absolute bottom-2.5 right-2.5 w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                        )}
                ...
                {/* Deploy New Website Wizard */}
                <div className="p-5 bg-white/2 border border-white/5 rounded-2xl space-y-4">
                  <h4 className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest font-mono flex items-center gap-1.5">
                    <Plus className="w-4 h-4 text-indigo-500" />
                    Deploy New Dynamic Website
                  </h4>
                  <form onSubmit={handleCreateSaaSWebsite} className="space-y-3.5">
                    <div>
                      <label className="text-[9px] text-slate-400 uppercase block mb-1">Website Title Name</label>
                      <input 
                        type="text" 
                        value={saasName} 
                    ...
                    <div>
                      <label className="text-[9px] text-slate-400 uppercase block mb-1">Industry Template Class</label>
                      <select
                        value={saasCategory}
                    ...
                    <div>
                      <label className="text-[9px] text-slate-400 uppercase block mb-1">Font family</label>
                      <select
                        value={saasFont}
                        onChange={(e) => setSaasFont(e.target.value)}
                        className="w-full bg-slate-950 border border-white/10 text-xs p-2 rounded-lg text-white"
                      >
                    ...
                    <button
                      type="submit"
                      className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs py-2.5 rounded-xl uppercase tracking-wider transition-all active:scale-95 shadow-md shadow-indigo-500/20"
                    >
            ...
            {activeTab === 'theme_manager' && (
              <div className="space-y-6 animate-in fade-in duration-300 text-left pb-12">
                <div className="flex justify-between items-center border-b border-white/5 pb-2">
                  <div>
                    <h3 className="font-bold text-sm text-indigo-400 font-mono uppercase tracking-wider">Dynamic Theme Engine</h3>
            ...
            {activeTab === 'content_manager' && (
              <div className="space-y-6 animate-in fade-in duration-300 text-left pb-12">
                <div className="flex justify-between items-center border-b border-white/5 pb-2">
                  <div>
                    <h3 className="font-bold text-sm text-indigo-400 font-mono uppercase tracking-wider">Content Manager</h3>
                    <p className="text-[10px] text-slate-400">Manage site copy, section configurations, and dynamic listings.</p>
                  </div>
                  <button 
                    onClick={handleSaveConfig}
                    className="bg-indigo-600 hover:bg-indigo-700 text-white text-[10px] font-bold px-3 py-1.5 rounded-lg transition-colors shadow"
                ...
                <div className="p-4 bg-indigo-500/5 border border-indigo-500/10 rounded-2xl space-y-3">
                  <span className="text-[9px] font-bold text-indigo-400 font-mono uppercase block">Hero Slogan & Branding Configuration</span>
                  <div className="space-y-3">
                    <div>
                      <label className="text-[9px] text-slate-400 block mb-0.5">Website Name / Logo Text</label>
                      <input 
                        type="text" 
                    ...
                    <div>
                      <label className="text-[9px] text-slate-400 block mb-0.5">Hero Main Heading</label>
                      <input 
                        type="text" 
                    ...
                    <div>
                      <label className="text-[9px] text-slate-400 block mb-0.5">Hero Subheading</label>
                      <textarea 
                        ...
                        className="w-full bg-slate-950 border border-white/10 text-xs px-3 py-2 rounded-lg h-20 text-white" 
                      />
                    </div>
                    <div>
                      <label className="text-[9px] text-slate-400 block mb-0.5">Hero Button CTA Label</label>
                      <input 
                        type="text" 
                    ...
                    <div>
                      <label className="text-[9px] text-slate-400 block mb-0.5">Hero Showcase Image URL</label>
                      <input 
                        type="text" 
                ...
                <div className="p-4 bg-emerald-500/5 border border-emerald-500/10 rounded-2xl space-y-3">
                  <span className="text-[9px] font-bold text-emerald-400 font-mono uppercase block">MOCK PAYMENT CONFIGURATION</span>
                  <p className="text-[9px] text-slate-400">Controls UPI IDs and banking destinations shown in the preview window sandbox checkouts.</p>
                  
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div>
                      <label className="text-[8px] text-slate-400">Active UPI VPA Address</label>
                      <input 
                        type="text" 
                        ...
                        className="w-full bg-slate-950 border border-white/10 text-[10px] px-2 py-1.5 rounded-lg text-white font-mono" 
                      />
                    </div>
                    <div>
                      <label className="text-[8px] text-slate-400">MOCK Bank Account</label>
                      <input 
                        type="text" 
                ...
                <button
                  ...
                  className="w-full border border-rose-500/20 hover:bg-rose-500/10 text-rose-400 text-[10px] font-bold py-2.5 rounded-xl transition-all font-mono"
                >
                  ⚠️ Restore default template configurations
                </button>
              </div>
            )}
            ...
            {activeTab === 'analytics' && (
              <div className="space-y-6 animate-in fade-in duration-300 text-left">
                <div className="space-y-1">
                  <h3 className="font-bold text-sm text-indigo-400 font-mono uppercase tracking-wider">Analytics & Webhooks</h3>
                  <p className="text-[10px] opacity-80 text-slate-400">Live edge metrics and Razorpay simulations.</p>
                </div>

                {/* Telemetry metrics */}
                <div className="grid grid-cols-3 gap-3">
                  <div className="p-3 bg-slate-900/60 border border-white/5 rounded-xl text-center space-y-1">
                    <span className="text-[8px] text-slate-500 uppercase font-mono block">CPU Usage</span>
                    <span className="text-md font-bold text-emerald-400 font-mono">42%</span>
                  </div>
                  <div className="p-3 bg-slate-900/60 border border-white/5 rounded-xl text-center space-y-1">
                    <span className="text-[8px] text-slate-500 uppercase font-mono block">Database Latency</span>
                    <span className="text-md font-bold text-white font-mono">18ms</span>
                  </div>
                  <div className="p-3 bg-slate-900/60 border border-white/5 rounded-xl text-center space-y-1">
                    <span className="text-[8px] text-slate-500 uppercase font-mono block">Mem Heap</span>
                    <span className="text-md font-bold text-indigo-400 font-mono">242MB</span>
                  </div>
                </div>

                {/* Interactive chart visualizer */}
                <div className="p-4 bg-slate-900/60 border border-white/5 rounded-2xl space-y-3">
                  <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest font-mono block">Mock Traffic Diagnostics</span>
                  <div className="h-28 flex items-end justify-between gap-1 pt-4 border-b border-white/10 pb-1">
                    {[34, 52, 18, 90, 78, 64, 85, 45, 95, 60, 40, 75].map((val, idx) => (
                      <div key={idx} className="flex-1 bg-indigo-500/10 rounded-t hover:bg-indigo-500 transition-colors relative group" style={{ height: `${val}%` }}>
                        <span className="absolute bottom-full left-1/2 -translate-x-1/2 bg-slate-950 text-white text-[7px] px-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap mb-1">
                          {val}K hits
                        </span>
                      </div>
                    ))}
                ...
                <div className="p-4 bg-emerald-500/5 border border-emerald-500/10 rounded-2xl space-y-4">
                  <div>
                    <h4 className="text-[10px] font-bold text-emerald-400 uppercase tracking-widest font-mono">Razorpay simulation events</h4>
                    <p className="text-[10px] text-slate-400 leading-tight mt-1">Triggers mock payment payloads to verify the WebSocket notification dispatchers in the preview frames.</p>
                  </div>
                  <form onSubmit={handleManualPaymentSimulation} className="space-y-3">
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <label className="text-[8px] text-slate-400 block mb-0.5">Amount (INR)</label>
                        <input 
                          type="text" 
                          value={paymentAmount} 
                          onChange={(e) => setPaymentAmount(e.target.value)} 
                          className="w-full bg-slate-950 border border-white/10 text-xs px-2.5 py-1.5 rounded-lg text-white font-mono" 
                          required
                        />
                      </div>
                      <div>
                        <label className="text-[8px] text-slate-400 block mb-0.5">Mock User Name</label>
                        <input 
                          type="text" 
                          value={paymentUser} 
                          onChange={(e) => setPaymentUser(e.target.value)} 
                          className="w-full bg-slate-950 border border-white/10 text-xs px-2.5 py-1.5 rounded-lg text-white" 
                          required
                        />
                      </div>
                    </div>
                    <div>
                      <label className="text-[8px] text-slate-400 block mb-0.5">Selected Offer / Plan</label>
                      <input 
                        type="text" 
                        value={paymentPlan} 
                    ...
                    <button
                      type="submit"
                      className="w-full bg-emerald-600 hover:bg-emerald-700 text-white text-[10px] font-bold py-2 rounded-xl flex items-center justify-center gap-1.5 shadow"
                    >
                      <CreditCard className="w-3.5 h-3.5" />
                      Dispatch Razorpay Webhook Broadcast
                    </button>
                  </form>
                </div>
              </div>
            ...
            {activeTab === 'users' && (
              <div className="space-y-6 animate-in fade-in duration-300 text-left">
                <div className="space-y-1">
                  <h3 className="font-bold text-sm text-indigo-400 font-mono uppercase tracking-wider">Registered Directory</h3>
                  <p className="text-[10px] opacity-80 text-slate-400">View registered students and configure custom roles.</p>
                </div>

                {/* Provision New User account form */}
                <div className="p-4 bg-slate-900/60 border border-white/5 rounded-2xl space-y-3">
                  <h4 className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest font-mono flex items-center gap-1">
                    <UserPlus className="w-3.5 h-3.5" /> Provision New User
                  </h4>
                  <form onSubmit={handleProvisionUser} className="space-y-3">
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <label className="text-[8px] text-slate-400 block mb-0.5">Email Address</label>
                        <input 
                          type="email" 
                          value={newUserEmail} 
                          onChange={(e) => setNewUserEmail(e.target.value)} 
                          className="w-full bg-slate-950 border border-white/10 text-xs px-2.5 py-1.5 rounded-lg text-white font-mono" 
                          placeholder="e.g. stud@jabalpur.gov"
                          required
                        />
                      </div>
                      <div>
                        <label className="text-[8px] text-slate-400 block mb-0.5">Full Name</label>
                        <input 
                          type="text" 
                          value={newUserName} 
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <label className="text-[8px] text-slate-400 block mb-0.5">Secure Password</label>
                        <input 
                          type="password" 
                      ...
                      <div>
                        <label className="text-[8px] text-slate-400 block mb-0.5">Assigned Role</label>
                        <select
                          value={newUserRole}
                          onChange={(e) => setNewUserRole(e.target.value)}
                          className="w-full bg-slate-950 border border-white/10 text-xs p-1.5 rounded-lg text-white font-mono"
                        >
                    ...
                    <button
                      type="submit"
                      className="w-full bg-indigo-600 hover:bg-indigo-700 text-white text-[10px] font-bold py-2 rounded-xl flex items-center justify-center gap-1.5 shadow"
                    >
                      <UserCheck className="w-3.5 h-3.5" />
                      Provision Credentials Profile
                    </button>
                  </form>
                </div>
                ...
                <div className="space-y-2">
                  <span className="text-[9px] font-bold text-slate-500 uppercase tracking-wider block font-mono">Active Users Directory ({users.length})</span>
                  <div className="space-y-1.5 max-h-[220px] overflow-y-auto pr-1">
                    {users.map((u) => (
                      <div key={u._id} className="p-3 bg-slate-900/60 border border-white/5 rounded-xl flex items-center justify-between text-xs">
                        <div className="truncate pr-2">
                          <span className="font-bold text-white block truncate">{u.name || 'Anonymous User'}</span>
                          <span className="text-[9px] font-mono text-slate-400 block truncate">{u.email}</span>
                        </div>
                        <span className={`text-[8px] font-bold font-mono px-2 py-0.5 rounded-full uppercase shrink-0 ${
                          u.role === 'admin' 
            ...
            {activeTab === 'settings' && (
              <div className="space-y-6 animate-in fade-in duration-300 text-left">
                <div className="space-y-1">
                  <h3 className="font-bold text-sm text-indigo-400 font-mono uppercase tracking-wider">Console Settings</h3>
                  <p className="text-[10px] opacity-80 text-slate-400">Configure global workspace preferences and connectivity overrides.</p>
                </div>
                ...
                <div className="p-4 bg-slate-900/60 border border-white/5 rounded-2xl space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <span className="text-xs font-bold text-white block">Acoustic Audio Chimes</span>
                      <span className="text-[9px] text-slate-400">Play hover sound effects and alert beeps.</span>
                    </div>
                    <button
                      onClick={() => setSoundEnabled(!soundEnabled)}
                      className={`w-10 h-6 rounded-full transition-all relative flex items-center p-0.5 border ${
                        soundEnabled ? 'bg-indigo-600 border-indigo-500 justify-end' : 'bg-slate-950 border-white/10 justify-start'
                      }`}
                    >
                      <span className="w-4.5 h-4.5 rounded-full bg-white block shadow" />
                    </button>
                  </div>

                  <div className="flex items-center justify-between">
                    <div>
                      <span className="text-xs font-bold text-white block">Console Glass Theme</span>
                      <span className="text-[9px] text-slate-400">Toggle dark obsidian / light glass dashboard style.</span>
                    </div>
                    <button
                      onClick={() => setLightMode(!lightMode)}
                      className={`w-10 h-6 rounded-full transition-all relative flex items-center p-0.5 border ${
                        lightMode ? 'bg-indigo-600 border-indigo-500 justify-end' : 'bg-slate-950 border-white/10 justify-start'
                      }`}
                    >
                      <span className="w-4.5 h-4.5 rounded-full bg-white block shadow" />
                    </button>
                ...
                <div className="p-4 bg-indigo-950/20 border border-indigo-500/10 rounded-2xl space-y-3 text-[11px] font-mono">
                  <span className="text-[9px] font-bold text-indigo-400 uppercase tracking-widest block">System Node Coordinates</span>
                  ...
                  <div className="space-y-1.5 text-slate-400">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-emerald-400 font-semibold">{serverStatus.mode} (OK)</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span>Socket Broadcast:</span>
                      <span className="text-indigo-400 font-semibold">Linked (Full Duplex)</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span>Processor Node:</span>
                      <span className="text-white font-semibold">Tilwara-Bhedaghat Edge</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        {/* Right Side: Preview Pane (8 columns) */}
        <div className="xl:col-span-8 bg-slate-900 flex flex-col overflow-hidden relative border-l border-white/5">
          ...
          <div className="h-14 border-b border-white/5 bg-slate-950 px-6 flex items-center justify-between shrink-0 font-mono">
            <div className="flex items-center gap-2 text-xs font-bold text-indigo-400 uppercase tracking-wider">
              ...
              <span>Website Preview Window</span>
            </div>
          ...
          {/* Iframe Device shell */}
          <div className="flex-1 p-8 flex items-center justify-center overflow-auto bg-slate-950/80">
            <div 
              style={{ 
            ...
            >
              ...
              {/* Iframe target loading the preview route */}
              <iframe
                ref={iframeRef}
                src="/preview"
                className="flex-1 w-full border-none bg-slate-950"
                title="Dynamic Website Engine Preview Frame"
              />
            </div>
          </div>
